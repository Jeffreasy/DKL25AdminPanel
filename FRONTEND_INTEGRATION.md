...
### 1. Token Management
- Store auth token as `auth_token` in localStorage (20 minute expiry)
- Store refresh token as `refresh_token` in localStorage (7 day expiry)
- Implement automatic token refresh (see TokenManager & TokenRefreshScheduler)
- Clear tokens on logout
- Token keys: `auth_token`, `refresh_token`, `token_expires_at`
