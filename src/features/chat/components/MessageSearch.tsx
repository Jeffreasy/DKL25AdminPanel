...
export function MessageSearch({ isOpen, onClose, onSelectMessage }: MessageSearchProps) {
  ...
  return (
    <div className={`fixed inset-0 ${cc.overlay.medium} flex items-start justify-center pt-20 z-50`}>
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[80vh] flex flex-col border border-gray-200 dark:border-gray-700">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto">
          {query.length < 3 ? (
            ...
            <div className={`${cc.spacing.container.sm} ${cc.spacing.section.md}`}>
              {[...Array(5)].map((_, i) => (
                <div key={i} className="animate-pulse">
                  <div className={`flex items-center ${cc.spacing.gap.md} ${cc.spacing.container.md}`}>
            ...
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {searchResults.map((message) => (
                <div
                  ...
                  className={`${cc.spacing.container.sm} hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer ${cc.transition.colors}`}
                >
