...
export function PhotoDetailsModal({ photo, onClose, onUpdate, albums, allPhotos, onNavigate }: PhotoDetailsModalProps) {
  ...
  const handleVisibilityToggle = async () => {
    try {
      setLoading(true)
      await photoApiClient.updatePhoto(photo.id, { visible: !photo.visible })
  ...
  const handleSave = async () => {
    try {
      setLoading(true)
      await photoApiClient.updatePhoto(photo.id, {
              ...
              {allPhotos && (
                <div className="lg:hidden absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-4">
                  <button
                    onClick={handlePrevious}
                    disabled={!hasPrevious}
                    className={`w-12 h-12 rounded-full flex items-center justify-center ${cc.transition.normal} ${
                      hasPrevious
              ...
              {allPhotos && (
                <>
                  {hasPrevious && (
                    <button
                      onClick={handlePrevious}
                      className={`hidden lg:flex absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 hover:bg-black/70 text-white rounded-full items-center justify-center ${cc.transition.normal} ${cc.hover.fadeIn}`}
                      title="Vorige foto (←)"
                    >
                      <ChevronLeftIcon className="w-6 h-6" />
                    </button>
                  ...
                  {hasNext && (
                    <button
                      onClick={handleNext}
                      className={`hidden lg:flex absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-black/50 hover:bg-black/70 text-white rounded-full items-center justify-center ${cc.transition.normal} ${cc.hover.fadeIn}`}
                      title="Volgende foto (→)"
                    >
                      <ChevronRightIcon className="w-6 h-6" />
                    </button>
              ...
              <div className="flex-1 overflow-y-auto p-6">
                {isEditing ? (
                  ...
                  <div className={cc.spacing.section.md}>
                    ...
                    <div className={`${cc.grid.detailTwo()} gap-4`}>
                      <div>
                        <h4 className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">
                          Status
                        </h4>
                    ...
                    <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                      ...
                      <div className={`${cc.grid.detailTwo()} gap-4 text-sm`}>
                        <div>
