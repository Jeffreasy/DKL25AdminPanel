...
import { usePhotoActions, usePhotoSelection } from '../hooks'
...
export function PhotoGrid({
  ...
  if (loading) {
    return <LoadingGrid variant="photos" count={12} className="gap-3 md:gap-4 lg:gap-5" />
  }
      ...
      <div className={`${cc.grid.photos()} gap-3 md:gap-4 lg:gap-5`}>
        {photos.map(photo => {
          ...
          return (
            <div
              key={photo.id}
              className={`group relative aspect-square rounded-xl overflow-hidden bg-gray-50 dark:bg-gray-800/50 border-2 transition-all ${
                isSelected
            ...
            >
              ...
              <div className="relative w-full h-full">
                ...
                {/* Action Buttons */}
                <div className={`photo-grid-actions absolute inset-0 flex items-center justify-center gap-2 p-3 ${cc.hover.fadeIn}`}>
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`p-2 rounded-full bg-white/90 backdrop-blur-sm text-gray-700 hover:bg-white shadow-lg ${cc.hover.scaleButton}`}
                    ...
                    <button
                      ...
                      className={`p-2 rounded-full bg-red-500/90 backdrop-blur-sm text-white hover:bg-red-500 shadow-lg ${cc.hover.scaleButton}`}
                ...
                {/* Photo Info Overlay */}
                <div className={`absolute bottom-0 left-0 right-0 p-3 ${cc.overlay.gradient.bottom} ${cc.hover.fadeIn}`}>
